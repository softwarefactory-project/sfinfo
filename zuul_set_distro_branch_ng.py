...
class ZuulSetDistroBranch(zuul_koji_lib.App):
    ...
    def update_repo_def(self, resources, raw_resources, name, branch):
        orepo = resources['resources']['repos'].get(name)
        if not orepo:
            self.log.info("Repo %s not found in the current resources file. Skip !" % name)
            return
        ...
        repo[name]['branches'][branch] = 'HEAD'
